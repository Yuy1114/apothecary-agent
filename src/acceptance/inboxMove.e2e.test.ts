...
beforeEach(async () => {
  ...
  await writeFile(abs("_inbox/idea.md"), "# Idea\n\na captured idea", "utf8");
  await writeFile(abs("_inbox/README.md"), "## 笔记索引\n\n- [Idea](idea.md) — 2026/7/1\n", "utf8");
...
describe("inbox move end-to-end", () => {
  it("approving a move relocates the note and keeps every layer consistent", async () => {
    const proposal = await createProposal(vault, {
      ...
      payload: { from: "_inbox/idea.md", to: "notes/idea.md" },
    });

    // Before approval nothing moves.
    expect(await access(abs("_inbox/idea.md")).then(() => true)).toBe(true);
    ...
    await expect(access(abs("_inbox/idea.md"))).rejects.toBeDefined();
    ...
    expect(await readFile(abs("_inbox/README.md"), "utf8")).not.toContain("(idea.md)");
    expect(await readFile(abs("notes/README.md"), "utf8")).toContain("(idea.md)");

    // Search index kept in sync both ways.
    expect(removeFromIndex).toHaveBeenCalledWith("_inbox/idea.md");
